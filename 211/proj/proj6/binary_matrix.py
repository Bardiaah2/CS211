...
def submatrix(matrix, r_loc, c_loc):
    ...
    (r1, r2), (c1, c2) = r_loc, c_loc
    rows = matrix[r1:r2]
    submatrix = [row[c1:c2] for row in rows]
    return submatrix
...
def same_bits(bin_mat):
    ...
    per = bin_mat[0][0]
    return all([all([(lambda x: x == per)(x) for x in row]) for row in bin_mat])
...
if __name__ == "__main__":
    ...
    binary_file = 'test.txt'  # in coding rooms change it to "images/test.txt"
